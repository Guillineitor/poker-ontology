...
/**
 * TestViabilidadRazonadoresOWL compara los razonadores OWL HermiT, Openllet (Pellet) y JFact (FaCT++)
 * sobre ontologías bajo el dominio del juego Poker Texas Hold'em.
 * HermiT se ejecuta sin límite de tiempo. Openllet y JFact tienen un timeout de 60 minutos (explicación en el README de la carpeta razonamiento).
 *
 * Uso:
 *   java -cp target/poker-reasoner-1.0-SNAPSHOT-jar-with-dependencies.jar poker.TestViabilidadRazonadoresOWL <ontologia.ttl> <instancias.ttl>
 *
 * Se puede designar a la ejecución del experimento una cantidad inicial y máxima de memoria RAM, como por ejemplo:
 *
 *   java -Xms28g -Xmx30g -cp target/poker-reasoner-1.0-SNAPSHOT-jar-with-dependencies.jar poker.TestViabilidadRazonadoresOWL <ontologia.ttl> <instancias.ttl>
 *
 ...
 *
 * Salida CSV:
 *   Se genera un único archivo {@code <ontologia>_benchmark_<timestamp>.csv} con dos secciones:
 *   una tabla de métricas por razonador y una tabla de clasificación individual.
 *   Los razonadores con TIMEOUT aparecen en la tabla de métricas con sus celdas vacías
 *   y la causa anotada; se omiten de la tabla de clasificación individual.
 */
public class TestViabilidadRazonadoresOWL {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length != 2) {
            System.err.println(RED
                + "Uso: java -cp target/poker-reasoner-1.0-SNAPSHOT-jar-with-dependencies.jar poker.TestViabilidadRazonadoresOWL <ontologia.ttl> <instancias.ttl>"
                + RESET);
            System.exit(1);
        }
        BASE_TTL = args[0];
    ...
    private static void imprimirTabla(List<ResultadoBenchmark> resultados) {
        ...
        for (ResultadoBenchmark r : resultados) {
            if (r.error != null) {
                System.out.printf("%-22s │ %s%n", r.nombre,
                    RED + "TIMEOUT/ERROR: " + r.error + RESET);
                continue;
            }
        ...
        for (String clase : CLASES_MANO) {
            ...
            for (ResultadoBenchmark r : resultados) {
                if (r.error != null) {
                    ...
                    String val = (n > 0 ? GREEN : "") + n + (n > 0 ? RESET : "");
     ...
     * si no existe y escribe un único archivo con las dos tablas.
     *
     * @param resultados lista de resultados, uno por razonador.
     */
